    ...
    public boolean preHandle(
        ...
        if (jwtProperties != null) {
            if (jwtProperties.getUserCheck()) {
                ...
                // 2、校验令牌
                try {
                    log.info("user - 校验token:{}", token);
                    Claims claims = JwtUtil.parseJWT(jwtProperties.getUserSecretKey(), token);
                    String s = claims.get(JwtClaimsConstant.USER_ID).toString();
                    long id = Long.parseLong(s);
                    log.info("user - 登录用户:{}", id);
                ...
                // 2、校验令牌
                try {
                    log.info("admin - 校验token:{}", token);
                    Claims claims = JwtUtil.parseJWT(jwtProperties.getAdminSecretKey(), token);
                    String s = claims.get(JwtClaimsConstant.USER_ID).toString();
                    long id = Long.parseLong(s);
                    log.info("admin - 登录用户:{}", id);
                    // 3、令牌校验通过，放行请求，继续执行后续处理器（Controller方法）
                } catch (Exception ex) {
                    // 4、令牌校验不通过，响应401 Unauthorized状态码，并阻止执行后续处理器
                    response.setStatus(HttpServletResponse.SC_UNAUTHORIZED); // 设置401状态码
